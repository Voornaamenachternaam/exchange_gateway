...
This guide configures Cloudflare Tunnel to expose the Exchange Gateway container via HTTPS. cloudflared runs natively on Ubuntu Server 24.04 LTS and connects to Cloudflare. All configuration is done via environment variables in Docker Compose.
...
Cloudflare Tunnel (encrypted HTTP/2)
        ↓
cloudflared (host: Ubuntu Server 24.04 LTS)
        ↓
...
For path-based routing (optional), expand **Path** and add regex:
```
(^/EWS.*|^/ews.*|^/autodiscover.*|^/Microsoft-Server-ActiveSync.*|^/OAB.*|^/Autodiscover.*|^/health.*)
```
...
# Get from: Networks → Tunnels → Your Tunnel → Access Token
...
|----------|-------------|----------|
| `TUNNEL_TOKEN` | Tunnel access token from dashboard | Yes |
| `CLOUDFLARED_TAG` | cloudflared image tag | No |
| `CLOUDFLARED_LOG_LEVEL` | Log verbosity | No |
...
# Test locally
curl http://exchange-gateway:8134/health
```
