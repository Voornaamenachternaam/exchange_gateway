...
pub async fn register_device(config: &AppConfig, user: &str, device_id: &str) {
    ...
    let _ = client.post(&config.db_api_url).bearer_auth(&config.db_auth_token).json(&body).send().await;
}

pub async fn get_sync_state(config: &AppConfig, user: &str, device_id: &str, coll: &str) -> Option<String> {
    let client = reqwest::Client::new();
    let body = json!({
        "query": "SELECT jmap_state FROM sync_state WHERE user_email = ? AND device_id = ? AND collection_id = ?",
        "params": [user, device_id, coll]
    });
    let res = client.post(&config.db_api_url).bearer_auth(&config.db_auth_token).json(&body).send().await.ok()?;
    let json: serde_json::Value = res.json().await.ok()?;
    
    // Path matches Worker wrapper: { "result": [ { "results": [...] } ] }
    json["result"][0]["results"][0]["jmap_state"].as_str().map(String::from)
}

pub async fn update_sync_state(config: &AppConfig, user: &str, device_id: &str, coll: &str, key: &str, state: &str) {
    let client = reqwest::Client::new();
    let body = json!({
...
pub async fn update_ews_sync_state(config: &AppConfig, user: &str, folder: &str, state: &str, jmap_state: &str) {
    let client = reqwest::Client::new();
    let body = json!({
