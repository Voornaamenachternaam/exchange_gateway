...
impl StalwartOofManager {
    ...
    pub fn new(
        ...
        if admin_base.is_empty() {
            return Err(OofError::ConfigError(
                "Admin base URL is required".to_string(),
            ));
        }
    ...
    fn get_current_script(&self, username: &str) -> Result<String, OofError> {
        ...
        let resp = req.send().map_err(|e| {
            if e.is_timeout() {
                ...
                OofError::HttpError(format!("HTTP {}", e.status().unwrap()))
            } else {
                OofError::NetworkError(format!("{}", e))
            }
        })?;
        ...
        if !resp.status().is_success() {
            return Err(OofError::HttpError(format!(
                "Stalwart returned {}",
                resp.status()
            )));
        }
        ...
        resp.text()
            .map_err(|e| OofError::HttpError(format!("Failed to read body: {}", e)))
    }
