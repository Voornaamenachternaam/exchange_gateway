...
pub async fn handle_ews(
    ...
    let (auth_user, auth_pass) =
        parse_basic_auth(&headers).unwrap_or((String::new(), String::new()));
...
async fn handle_create_item(
    ...
    match ews_marshaller::ews_calendaritem_to_ics(xml) {
        Ok(ics) => {
            ...
            match caldav
                .put_event(&coll, &resource_name, &ics, owner, password)
                .await
            {
                Ok(etag) => {
                    let resource_href = format!("{}/{}", coll.trim_end_matches('/'), resource_name);
                    let server_id =
                        sync::generate_server_id(&state.cfg.hmac_secret, &resource_href);
                    let _ = state
                        .storage
                        .upsert_item_map(
                            owner,
                ...
                Err(e) => {
                    (StatusCode::BAD_GATEWAY, format!("CalDAV put error: {}", e)).into_response()
                }
            }
        }
        Err(e) => (
