...
pub struct SendEmailParams<'a> {
    ...
    /// Client-supplied Message-ID (without angle brackets) to preserve on the
    /// wire; a fresh ID is generated by the framework when absent.
...
impl SmtpClient {
    ...
    pub async fn send_email(&self, params: SendEmailParams<'_>) -> anyhow::Result<SendResult> {
        ...
        // brackets (JMAP normalisation); restore them for the wire. lettre's
        // MessageBuilder::header replaces a previously set header of the same
        // name, so all IDs must be combined into one header line.
        if let Some(irt) = params.in_reply_to {
            let value = irt
                ...
                .join(" ");
            if !value.is_empty() {
                builder = builder.header(lettre::message::header::InReplyTo::from(value));
            }
        }
        ...
        // Preserve a client-supplied Message-ID verbatim on the wire.
        if let Some(mid) = params.message_id {
            let id = mid.trim_matches(|c| c == '<' || c == '>');
            if !id.is_empty() {
                builder =
                    builder.header(lettre::message::header::MessageId::from(format!("<{id}>")));
            }
        }
