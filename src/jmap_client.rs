...
pub struct JmapEvent {
    ...
    // We convert to Map to `push_event` and from `Map` in `from_jmap_json`.
...
pub async fn get_session(url: &str, user: &str, pass: &str) -> Result<JmapSession, JmapError> {
    ...
    // Use internal URL for Docker network, let session_url = if url.ends_with("/.well-known/jmap") || url.ends_with("/jmap") {
        url.to_string()
    } else {
    ...
    if !res.status().is_success() {
        ...
        let access_token = format!("Basic {}", base64::engine::general_purpose::STANDARD.encode(format!("{}:{}", user, pass));
    ...
    Err(e) => {
        tracing::error!("JMAP session error: {:?}", e);
        Err(JmapError::Request(e))
    }
}
...
pub async fn update_participant_status(
    ...
    // We need to update this participant within the participants map
    ...
    if let Some(participant) = current_participants.get_mut(user_email) {
        ...
        current_participants[user_email.to_string()] = participant_patch;
    }
    
    // Update the event with new participants map
