...
pub async fn get_calendar_state(
    ...
    let state = json["methodResponses"][0][1]["state"]
        .as_str()
...
impl JmapEvent {
    fn from_jmap_json(value: &serde_json::Value) -> Result<Self, JmapError> {
        let map = value
            .as_object()
            .ok_or_else(|| JmapError::Method("Invalid JSON object".to_string()))?;
        ...
        let end = map
            ...
            .and_then(|v| v.as_str().map(String::from))
            .unwrap_or_default();
        let location = map.get("location").and_then(|v| {
