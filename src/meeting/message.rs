...
use crate::calendar::{CalendarItem, Attendee};
use crate::meeting::attendee::{AttendeeStatus, AttendeeRole};
...
impl MeetingMessageGenerator {
        ...
        if msg.message_type == MeetingMessageType::Request || 
           msg.message_type == MeetingMessageType::Update {
            ics.push_str(&format!("ORGANIZER;CN={}:mailto:{}\r\n", 
                escape_ical_text(msg.organizer_name.as_deref().unwrap_or(&msg.organizer_email)),
                msg.organizer_email
            ));
            ...
            && let (Some(start), Some(end)) = (msg.proposed_start, msg.proposed_end) {
                ...
                ics.push_str(&format!("DTEND;X-MS-OLK-ORIGINAL={}Z:{}Z\r\n",
    ...
    pub fn generate_ews_create_response(
        ...
        if !item.attendees.is_empty() {
            let required: Vec<&Attendee> = item.attendees.iter()
                .filter(|a| a.attendee_type.unwrap_or(1) == 1)
                .collect();
            let optional: Vec<&Attendee> = item.attendees.iter()
    ...
    pub fn generate_eas_meeting_request(&self, item: &CalendarItem, _server_id: &str) -> String {
        ...
        xml.push_str(&format!("<Calendar:Subject xmlns:Calendar=\"Calendar:\">{}</Calendar:Subject>",
            xml_escape(&item.subject)));
        xml.push_str(&format!("<Calendar:Location xmlns:Calendar=\"Calendar:\">{}</Calendar:Location>",
            xml_escape(&item.location)));
        xml.push_str(&format!("<Calendar:StartTime xmlns:Calendar=\"Calendar:\">{}</Calendar:StartTime>",
            item.start.format("%Y-%m-%dT%H:%M:%SZ")));
        xml.push_str(&format!("<Calendar:EndTime xmlns:Calendar=\"Calendar:\">{}</Calendar:EndTime>",
            item.end.format("%Y-%m-%dT%H:%M:%SZ")));
        ...
        if !item.attendees.is_empty() {
            ...
            for att in &item.attendees {
                ...
                xml.push_str(&format!("<Calendar:Email>{}</Calendar:Email>", xml_escape(&att.email)));
                if let Some(ref name) = att.name {
                    xml.push_str(&format!("<Calendar:Name>{}</Calendar:Name>", xml_escape(name)));
                }
                xml.push_str(&format!("<Calendar:AttendeeType>{}</Calendar:AttendeeType>",
        ...
        xml.push_str(&format!("<Calendar:MeetingStatus xmlns:Calendar=\"Calendar:\">{}</Calendar:MeetingStatus>",
        ...
        if let Some(ref organizer) = item.organizer_email {
            ...
            xml.push_str(&format!("<Calendar:Email>{}</Calendar:Email>", xml_escape(organizer)));
