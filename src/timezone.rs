...
use windows_timezones::{WindowsTimezone, WINDOWS_TIMEZONES};
...
pub fn eas_timezone_blob_to_iana(b64: &str) -> Option<String> {
    ...
    if let Some(iana) = windows_name_to_iana(&std_name)
        .or_else(|| windows_name_to_iana(&dst_name))
    {
...
fn windows_name_to_iana(name: &str) -> Option<&'static str> {
    ...
    if let Some(tz) = WINDOWS_TIMEZONES.iter().find(|tz| {
        tz.name.eq_ignore_ascii_case(n)
            || tz.std_name.eq_ignore_ascii_case(n)
            || tz.dst_name.as_ref().map(|d| d.eq_ignore_ascii_case(n)).unwrap_or(false)
    }) {
        return Some(tz.iana);
    }

    for tz in WINDOWS_TIMEZONES.iter() {
        if tz.std_name.eq_ignore_ascii_case(n) || tz.name.eq_ignore_ascii_case(n) {
            return Some(tz.iana);
        }
    }
    ...
    for &(pattern, iana) in FALLBACK_TABLE {
        if n_lower.contains(pattern) {
            return Some(iana);
        }
    }
    None
...
fn parse_utc_offset_name(name: &str) -> Option<&'static str> {
    ...
    if !n.contains("utc") && !n.contains("gmt") {
        return None;
    }
    for (pattern, iana) in UTC_OFFSET_PATTERNS {
...
const UTC_OFFSET_PATTERNS: &[(&str, &str)] = &[
    ("+01", "Etc/GMT-1"), ("+02", "Etc/GMT-2"), ("+03", "Etc/GMT-3"),
    ("+04", "Etc/GMT-4"), ("+05", "Etc/GMT-5"), ("+06", "Etc/GMT-6"),
    ("+07", "Etc/GMT-7"), ("+08", "Etc/GMT-8"), ("+09", "Etc/GMT-9"),
    ("+10", "Etc/GMT-10"), ("+11", "Etc/GMT-11"), ("+12", "Etc/GMT-12"),
    ("-01", "Etc/GMT+1"), ("-02", "Etc/GMT+2"), ("-03", "Etc/GMT+3"),
    ("-04", "Etc/GMT+4"), ("-05", "Etc/GMT+5"), ("-06", "Etc/GMT+6"),
    ("-07", "Etc/GMT+7"), ("-08", "Etc/GMT+8"), ("-09", "Etc/GMT+9"),
    ("-10", "Etc/GMT+10"), ("-11", "Etc/GMT+11"), ("-12", "Etc/GMT+12"),
...
fn iana_to_windows_params(iana: &str) -> Option<TzParams> {
    if let Some(tz) = WINDOWS_TIMEZONES.iter().find(|tz| tz.iana.eq_ignore_ascii_case(iana)) {
        let bias = tz.bias;
        let std_name = tz.std_name;
        let dst_name = tz.dst_name.as_deref().unwrap_or("");
        let (std_date, dst_date, std_bias, dst_bias) = if tz.dst_name.is_some() {
            (EU_STD, EU_DST, 0, -60)
        } else {
            (NO_DST, NO_DST, 0, 0)
        };
        return Some((bias, std_name, dst_name, std_date, dst_date, std_bias, dst_bias));
    }
    ...
    Some(match iana {
        ...
        | "Europe/Madrid" | "Europe/Stockholm" | "Europe/Brussels" | "Europe/Copenhagen"
        | "Europe/Vienna" | "Europe/Warsaw" | "Europe/Zagreb" | "Europe/Budapest" => {
            (-60, "W. Europe Standard Time", "W. Europe Daylight Time", EU_STD, EU_DST, 0, -60)
        }
        "Europe/Helsinki" | "Europe/Tallinn" | "Europe/Riga" | "Europe/Vilnius"
        | "Europe/Kyiv" | "Europe/Bucharest" | "Europe/Athens" | "Europe/Sofia" => {
            (-120, "FLE Standard Time", "FLE Daylight Time", EU_STD, EU_DST, 0, -60)
        }
...
pub fn ews_timezone_id_to_iana(tz_id: &str) -> Option<&'static str> {
    if let Some(tz) = WINDOWS_TIMEZONES.iter().find(|tz| tz.id.eq_ignore_ascii_case(tz_id)) {
        return Some(tz.iana);
    }
    windows_name_to_iana(tz_id)
}

pub fn iana_to_ews_timezone_id(iana: &str) -> Option<&'static str> {
    WINDOWS_TIMEZONES
        .iter()
        .find(|tz| tz.iana.eq_ignore_ascii_case(iana))
        .map(|tz| tz.id)
}
...
mod tests {
    ...
    #[test]
    fn ews_timezone_id_conversion() {
        assert_eq!(ews_timezone_id_to_iana("Pacific Standard Time"), Some("America/Los_Angeles"));
        assert_eq!(ews_timezone_id_to_iana("W. Europe Standard Time"), Some("Europe/Berlin"));
    }

    #[test]
    fn iana_to_ews_id_conversion() {
        assert_eq!(iana_to_ews_timezone_id("America/Los_Angeles"), Some("Pacific Standard Time"));
        assert_eq!(iana_to_ews_timezone_id("Europe/Berlin"), Some("W. Europe Standard Time"));
    }
}