...
impl SubscriptionManager {
    ...
    pub async fn get_receiver(
        &self,
        sub_id: &str,
    ) -> Option<broadcast::Receiver<NotificationEvent>> {
        let mut subs = self.subscriptions.lock().await;
        subs.remove(sub_id)
    }
...
static GLOBAL_SUBSCRIPTION_MANAGER: LazyLock<SubscriptionManager> =
    LazyLock::new(SubscriptionManager::new);
...
mod tests {
    ...
    async fn test_subscription_create_and_send() {
        ...
        mgr.send(NotificationEvent::NewMail {
