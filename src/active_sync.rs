...
pub async fn process_request(config: &AppConfig, xml: &str, headers: &HeaderMap) -> String {
    ...
    let device_id = headers.get("X-MS-DeviceId").and_then(|h| h.to_str().ok()).unwrap_or("unknown");

    let session = match jmap_client::get_session(&config.jmap_url, &user, &pass).await {
        Ok(s) => s,
        Err(e) => { tracing::error!("JMAP Auth failed: {}", e); return error_xml(500, "AuthFailed"); }
    };
    ...
    match command.as_str() {
        ...
        "ItemOperations" => {
            let req: ItemOpsReq = match quick_xml::de::from_str(xml) { Ok(r) => r, Err(_) => return error_xml(400, "BadRequest") };
        ...
        _ => { tracing::warn!("Unsupported EAS Command: {}", command); error_xml(400, "UnsupportedCommand") }
    }
}
...
async fn handle_send_mail(config: &AppConfig, xml: &str) -> String {
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(ref e)) => { if String::from_utf8_lossy(e.local_name().as_ref()) == "Mime" { in_mime = true; } }
            Ok(Event::End(ref e)) => { if String::from_utf8_lossy(e.local_name().as_ref()) == "Mime" { in_mime = false; } }
            Ok(Event::Text(t)) => { if in_mime { mime_content.push_str(&escape::unescape(std::str::from_utf8(&t).unwrap_or("")).unwrap_or_default()); } }
            Ok(Event::Eof) => break,
            _ => {}
        }
    }
    if mime_content.is_empty() { return r#"<SendMail xmlns="AirSync:"><Status>2</Status></SendMail>"#.to_string(); }
    ...
    let status = if let (Some(to), Some(from)) = (to_addr, from_addr) {
        let subject = re_subj.captures(&mime_content).and_then(|c| c.get(1)).map(|m| m.as_str().trim().to_string()).unwrap_or_default();
        let body_start = mime_content.find("\r\n\r\n").unwrap_or(0);
        let clean_body = mime_content.split_at(body_start).1.trim_start_matches("\r\n\r\n");
        ...
        let mailer = SmtpTransport::builder_dangerous(smtp_url.host_str().unwrap()).port(smtp_url.port().unwrap_or(25)).build();
        match mailer.send(&email) { Ok(_) => "1", Err(e) => { tracing::error!("SMTP Error: {}", e); "2" } }
    } else { "2" };

    format!(r#"<SendMail xmlns="AirSync:"><Status>{}</Status></SendMail>"#, status)
}

async fn handle_meeting_response(session: &jmap_client::JmapSession, xml: &str, user_email: &str) -> String {
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(ref e)) => { current_tag = String::from_utf8_lossy(e.local_name().as_ref()).to_string(); }
            Ok(Event::Text(t)) => {
                let text = escape::unescape(std::str::from_utf8(&t).unwrap_or("")).unwrap_or_default();
    ...
    if uid.is_empty() { return error_xml(400, "Missing UID"); }
    ...
    let status_str = match response_code { 1 => "accepted", 2 => "tentative", 3 => "declined", _ => "needs-action" };
    ...
    format!(r#"<MeetingResponse xmlns="AirSync:"><Result><Status>1</Status><CalendarId>{}</CalendarId></Result></MeetingResponse>"#, escape_xml(&event_id))
}
...
async fn handle_search(session: &jmap_client::JmapSession, xml: &str) -> String {
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(ref e)) => { current_tag = String::from_utf8_lossy(e.local_name().as_ref()).to_string(); }
            Ok(Event::Text(t)) => { if current_tag == "FreeText" { query = String::from_utf8_lossy(&t).to_string(); } }
            Ok(Event::Eof) => break,
            _ => {}
        }
    }
    let results = jmap_client::search_principals(&session.api_url, &session.access_token, &session.account_id, &query).await.unwrap_or_default();
    ...
    format!(r#"<Search xmlns="Search:"><Status>1</Status><Response><Store><Status>1</Status><Result>{}</Result></Store></Response></Search>"#, results_xml)
}
    ...
    for fetch in req.item_operations.fetch.into_iter().chain(req.item_operations.fetch_alt.into_iter()) {
        if let Some(store) = fetch.store {
            ...
            if let Some(id) = id_opt {
                if let Ok(event) = jmap_client::get_event_by_id(&session.api_url, &session.access_token, &session.account_id, &id).await {
                ...
                } else { results.push_str("<Response><Fetch><Status>6</Status></Fetch></Response>"); }
            }
        }
    }
    format!(r#"<ItemOperations xmlns="ItemOperations:"><Status>1</Status>{}</ItemOperations>"#, results)
}
...
async fn handle_sync(session: &jmap_client::JmapSession, config: &AppConfig, user: &str, device_id: &str, req: SyncRequest) -> String {
    ...
    if let Some(cmds) = coll.commands { process_client_commands(session, cmds, &config.timezone, user).await; }
    ...
    let (items_xml, new_sync_key) = if old_sync_key == "0" || prev_state.is_none() {
        ...
        for event in events { xml.push_str(&render_event_xml(event, "Add", &config.timezone)); }
        (xml, new_key)
    } else {
        let prev_jmap_state = prev_state.unwrap();
        if prev_jmap_state == current_jmap_state {
            return format!(r#"<Sync xmlns="AirSync:"><Collections><Collection><SyncKey>{}</SyncKey><CollectionId>{}</CollectionId><Status>1</Status></Collection></Collections></Sync>"#, escape_xml(&old_sync_key), escape_xml(&collection_id));
        }
    ...
    if new_sync_key != old_sync_key { db::update_sync_state(config, user, device_id, &collection_id, &new_sync_key, &current_jmap_state).await; }

    format!(r#"<Sync xmlns="AirSync:" xmlns:Calendar="Calendar:" xmlns:AirSyncBase="AirSyncBase:"><Collections><Collection><SyncKey>{}</SyncKey><CollectionId>{}</CollectionId><Status>1</Status><Commands>{}</Commands></Collection></Collections></Sync>"#, escape_xml(&new_sync_key), escape_xml(&collection_id), items_xml)
}
...
fn render_event_xml(event: jmap_client::JmapEvent, mode: &str, tz_str: &str) -> String {
    ...
    if let Some(attendees) = &event.participants {
        ...
        for att in attendees { attendees_xml.push_str(&format!(r#"<Calendar:Attendee><Calendar:Email>{}</Calendar:Email><Calendar:Name>{}</Calendar:Name></Calendar:Attendee>"#, escape_xml(&att.email), escape_xml(&att.name))); }
        attendees_xml.push_str("</Calendar:Attendees>");
    }
...
fn parse_rrule_to_eas(rrule: &str) -> String {
    ...
    let mut freq = "0"; let mut interval = "1"; let mut day_of_week = String::new();
    for part in parts {
        if part.starts_with("FREQ=") { freq = match &part[5..] { "DAILY" => "0", "WEEKLY" => "1", "MONTHLY" => "2", "YEARLY" => "3", _ => "0" }; }
        if part.starts_with("INTERVAL=") { interval = &part[9..]; }
        if part.starts_with("BYDAY=") { day_of_week = part[6..].split(',').map(|d| match d.trim() { "MO"=>"2", "TU"=>"4", "WE"=>"8", "TH"=>"16", "FR"=>"32", "SA"=>"64", "SU"=>"1", _=>"" }).collect::<Vec<&str>>().join(","); }
    }
    let day_xml = if day_of_week.is_empty() { String::new() } else { format!("<Calendar:DayOfWeek>{}</Calendar:DayOfWeek>", day_of_week) };
    format!(r#"<Calendar:Recurrence><Calendar:Type>{}</Calendar:Type><Calendar:Interval>{}</Calendar:Interval>{}</Calendar:Recurrence>"#, freq, interval, day_xml)
}

async fn process_client_commands(session: &jmap_client::JmapSession, cmds: Commands, tz_str: &str, _user: &str) {
    let tz: Tz = tz_str.parse().unwrap_or(chrono_tz::UTC);
    for add_cmd in cmds.add.unwrap_or_default() {
        ...
        let attendees: Vec<jmap_client::Participant> = data.attendees.map(|a| a.items.into_iter().map(|att| jmap_client::Participant { email: att.email, name: att.name, status: None }).collect()).unwrap_or_default();
        let event = jmap_client::JmapEvent {
            id: None, title: data.subject.unwrap_or_default(), start: start_utc, end: end_utc, location: data.location,
            description: data.body.map(|b| b.data), uid: data.uid.or(Some(Uuid::new_v4().to_string())),
            participants: if attendees.is_empty() { None } else { Some(attendees) }, is_all_day: data.all_day_event.unwrap_or(0) == 1,
            recurrence_rule: data.recurrence.map(|r| build_rrule(r)), updated: None,
        };
    ...
    for change_cmd in cmds.change.unwrap_or_default() {
        let id = change_cmd.server_id; let data = change_cmd.application_data; let mut patch = serde_json::Map::new();
        if let Some(s) = data.subject { patch.insert("title".into(), serde_json::json!(s)); }
        if let Some(l) = data.location { patch.insert("location".into(), serde_json::json!(l)); }
        if let Some(s) = data.start { patch.insert("start".into(), serde_json::json!(parse_local_to_utc(&s, tz))); }
        if let Some(e) = data.end { patch.insert("end".into(), serde_json::json!(parse_local_to_utc(&e, tz))); }
        if let Some(b) = data.body { patch.insert("description".into(), serde_json::json!(b.data)); }
        if !patch.is_empty() { let _ = jmap_client::patch_event(&session.api_url, &session.access_token, &session.account_id, &id, patch).await; }
    }
    if let Some(deletes) = cmds.delete { if !deletes.is_empty() { let ids: Vec<String> = deletes.into_iter().map(|d| d.server_id).collect(); let _ = jmap_client::destroy_events(&session.api_url, &session.access_token, &session.account_id, ids).await; } }
}

fn build_rrule(r: Recurrence) -> String {
    let freq = match r.r#type { 0 => "DAILY", 1 => "WEEKLY", 2 => "MONTHLY", 3 => "YEARLY", _ => "DAILY" };
    let mut parts = vec![format!("FREQ={}", freq)]; parts.push(format!("INTERVAL={}", r.interval));
    if let Some(dow) = r.day_of_week {
        let mut days = Vec::new(); if (dow&1)!=0 { days.push("SU"); } if (dow&2)!=0 { days.push("MO"); } if (dow&4)!=0 { days.push("TU"); } if (dow&8)!=0 { days.push("WE"); } if (dow&16)!=0 { days.push("TH"); } if (dow&32)!=0 { days.push("FR"); } if (dow&64)!=0 { days.push("SA"); }
        if !days.is_empty() { parts.push(format!("BYDAY={}", days.join(","))); }
    }
    parts.join(";")
}
...
async fn render_changes(session: &jmap_client::JmapSession, changes: jmap_client::JmapChanges, tz_str: &str) -> (String, String) {
    let mut xml = String::new(); let new_key = Uuid::new_v4().to_string();
    for id in &changes.destroyed { xml.push_str(&format!("<Delete><ServerId>{}</ServerId></Delete>", escape_xml(id))); }
    if !changes.updated.is_empty() {
        if let Ok(events) = jmap_client::get_events_by_ids(&session.api_url, &session.access_token, &session.account_id, &changes.updated).await {
            for event in events { xml.push_str(&render_event_xml(event, "Change", tz_str)); }
        }
    }
    (xml, new_key)
}
...
fn error_xml(code: i32, msg: &str) -> String { format!("<Error><Code>{}</Code><Message>{}</Message></Error>", code, msg) }

#[derive(Debug, Deserialize)] struct SyncRequest { #[serde(rename = "Collections")] collections: SyncCollections }
#[derive(Debug, Deserialize)] struct SyncCollections { #[serde(rename = "Collection")] collection: SyncCollection }
#[derive(Debug, Deserialize)] struct SyncCollection { #[serde(rename = "SyncKey")] sync_key: String, #[serde(rename = "CollectionId")] collection_id: String, #[serde(rename = "Commands", skip_serializing_if = "Option::is_none")] commands: Option<Commands> }
#[derive(Debug, Deserialize)] struct Commands { #[serde(rename = "Add", skip_serializing_if = "Option::is_none")] add: Option<Vec<AddCommand>>, #[serde(rename = "Change", skip_serializing_if = "Option::is_none")] change: Option<Vec<ChangeCommand>>, #[serde(rename = "Delete", skip_serializing_if = "Option::is_none")] delete: Option<Vec<DeleteCommand>> }
#[derive(Debug, Deserialize)] struct AddCommand { #[serde(rename = "ClientId")] client_id: String, #[serde(rename = "ApplicationData")] application_data: ApplicationData }
