...
fn parse_rrule_to_eas(rrule: &str) -> String {
    ...
    for part in parts {
        ...
        if let Some(val) = part.strip_prefix("FREQ=") {
            freq = match val {
                "DAILY" => "0", "WEEKLY" => "1", "MONTHLY" => "2", "YEARLY" => "3", _ => "0"
            };
        }
        ...
        if let Some(val) = part.strip_prefix("BYDAY=") {
            day_of_week = val.split(',').map(|d| match d.trim() { "MO"=>"2", "TU"=>"4", "WE"=>"8", "TH"=>"16", "FR"=>"32", "SA"=>"64", "SU"=>"1", _=>"" }).collect::<Vec<&str>>().join(",");
        }
    }
...
#[derive(Debug, Deserialize)] struct AddCommand { #[serde(rename = "ClientId")] _client_id: String, #[serde(rename = "ApplicationData")] application_data: ApplicationData }
