...
// ...
    if let Some(mut entry) = PING_CACHE.get_mut(&cache_key) {
        entry.heartbeat = heartbeat;
        entry.folders = folders.clone();
    } else {
        if PING_CACHE.len() >= 10000 {
            let keys: Vec<String> = PING_CACHE.iter().map(|r| r.key().clone()).take(1000).collect();
